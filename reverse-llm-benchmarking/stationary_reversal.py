...
def stationary_reverse_full_dist(
    ...
    for i in range(prefix_length):
        ...
        for batch_num in tqdm(range(total_batches)):
            ...
            batch_indices = (
                torch.arange(start_idx, end_idx).clamp(0, vocab_size - 1).to(device)
            )
            v_sentences = torch.cat(
                (batch_indices.unsqueeze(1), splus.repeat(batch_indices.size(0), 1)),
                dim=-1,
            )
            ...
            if end_idx > vocab_size:
                ...
                padded_stationary_dist = (
                    stationary_dist[-1].repeat(vocab_batch_size).to(device)
                )
                # Replace the beginning of this tensor with the actual values from stationary_dist for this batch
                padded_stationary_dist[:actual_vocab_batch_size] = stationary_dist[
                    start_idx : start_idx + actual_vocab_batch_size
                ]
            ...
            newlogprob_batch = (
                psp_given_v_batch + torch.log(stationary_dist[start_idx:end_idx]) - psp
            )
            vector_of_logprobs[
                prefix_length - i - 1, start_idx:end_idx
            ] = newlogprob_batch
        ...
        if renormalize_dist:
            vector_of_logprobs = torch.nn.functional.log_softmax(
                vector_of_logprobs, dim=-1
            )

    return vector_of_logprobs, splus
