...
model = GPTNeoXForCausalLM.from_pretrained("EleutherAI/pythia-160m").cuda()
...
eval_size = 50 #45 for 4.5ish hours using 6 weights
...
for prefix_weight in [0]+[0.02*3**i for i in range(4)]+[1]:
    ...
    reversal = reversal = ReversalLMPrior(model, reverse_model, tokenizer, batch_size=2000)

    for p,pair in enumerate(tqdm(toxic_stuff)):
        if len(reversal_loss)==eval_size: break
        prefix, suffix = pair
...
with open('/home/adi224/reverse-dynamics-nlp/reversal_results_toxic_50sample.pkl', 'wb') as f:
